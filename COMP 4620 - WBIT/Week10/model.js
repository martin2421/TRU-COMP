...
var URL = "mongodb://w4matanacio:w4matanacio136@127.0.0.1:27017/COMP4620_test";  // MongoDB
...
const ready = async function(callback)
{
    if (connected)
        ...
        try {
            ...
            conn = await MongoClient.connect(URL); // connect
            db = conn.db(); // database
            
            timerid = setTimeout(close, 2000);
            
            let list = await db.listCollections().toArray(); // get collections
