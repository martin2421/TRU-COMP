     ...
     <div id="layout-right">
          ...
          <div class='modal' id='modal-postaquestion'>
               ...
               <form method='get' action='w7_controller_student.php'>
                    ...
                    <input class='submit' id='submit-question' type='button' value='Submit'>
               </form>
          </div>
          ...
          <div class='modal' id='modal-searchquestions'>
               ...
               <form method='get' action='w7_controller_student.php'>
                    ...
                    <input class='submit' id='submit-search' type='button' value='Submit'>
               </form>
          </div>
...
<script>
     ...
     // Timeout
     var timer = setTimeout(timeout, 100000);
     ...
     function event_listener_mousemove_or_keydown() {
          clearTimeout(timer);
          timer = setTimeout(timeout, 100000);
     }
     ...
     // Post A Question - AJAX
     $(document).ready(function () {
          $('#submit-question').click(function () {
               var xhttp = new XMLHttpRequest(); // create an AJAX object
               xhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                         $('#modal-postaquestion').css('display', 'none');
                         $('#blanket').css('display', 'none');
                         $('#results').html(this.responseText);
                    }
               };

               var controller = "w7_controller_student.php";
               var query = "page=MainPage&command=PostAQuestion&Question=" + $('#question').val();
               xhttp.open("GET", controller + "?" + query, true);
               xhttp.send();
     ...
     // Search Question - AJAX
     $(document).ready(function () {
          $('#submit-search').click(function () {
               var xhttp = new XMLHttpRequest(); // create an AJAX object
               xhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                         $('#modal-searchquestions').css('display', 'none');
                         $('#blanket').css('display', 'none');
                         $('#results').html(this.responseText);
                    }
               };
               ...
               var controller = "w7_controller_student.php";
               var query = "page=MainPage&command=SearchQuestions&Search=" + $('#search-question').val();
               xhttp.open("GET", controller + "?" + query, true);
               xhttp.send();
